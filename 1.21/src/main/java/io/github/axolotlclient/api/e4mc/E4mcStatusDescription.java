...
public record E4mcStatusDescription(String levelName, String domain, ServerMetadata serverMetadata) {
	...
	public String write() {
		...
		if (domain != null && serverMetadata != null) {
			...
			fields.put("server_metadata", ServerMetadata.CODEC.encodeStart(JsonOps.INSTANCE, serverMetadata).getOrThrow());
		}
	...
	public static E4mcStatusDescription read(String json) {
		...
		var serverMetadata = obj.has("server_metadata") ? ServerMetadata.CODEC.parse(JsonOps.INSTANCE, obj.get("server_metadata")).getOrThrow() : null;
		return new E4mcStatusDescription(name, domain, serverMetadata);
	}

	public ServerInfo getServerData(String username) {
		var data = new ServerInfo(username, domain, ServerInfo.ServerType.OTHER);
